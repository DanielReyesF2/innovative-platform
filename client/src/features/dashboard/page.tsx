...
import { calcularWeightedPipeline, calcularWinRate, KANBAN_STAGES, SectionHeader } from "@/lib/comercial-constants";

export default function DashboardPage() {
  const [, navigate] = useLocation();
  const { kanbanProspectos, salesTeamData, currentUserName, userGreeting, isLoading, isError } = useComercialData();
      ...
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="bg-gradient-to-br from-[#1c2c4a] to-[#0D47A1] rounded-xl p-5 text-white">
          ...
          <div className="text-xs text-white/60 mt-1">
            Venta real: ${(ventasTotal / 1000000).toFixed(1)}M ·{" "}
            {presupuestoTotal > 0 ? Math.round((ventasTotal / presupuestoTotal) * 100) : 0}% cumplimiento
          </div>
        </div>
        ...
        <SectionHeader
          ...
          onLinkClick={() => navigate("/comercial")}
        />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4">
          ...
          <div className="lg:col-span-2 bg-white rounded-xl border border-[#e5e7eb] p-4">
            ...
            <div className="mt-2 pt-2 border-t border-[#e5e7eb] flex items-center justify-between text-[11px] text-[#6b7280]">
              ...
              <span>
                Ponderado:{" "}
                <span className="font-semibold text-[#00a8a8]">${(pipelinePonderado / 1000000).toFixed(1)}M</span>
              </span>
            </div>
          </div>
              ...
              {topDeals.length > 0 ? (
                topDeals.map((deal, idx) => {
                  ...
                  return (
                    ...
                    >
                      <div className="w-5 h-5 rounded-full bg-[#f3f4f6] flex items-center justify-center text-[10px] font-bold text-[#6b7280]">
                        {idx + 1}
                      </div>
