...
function BarValueLabel(props: {
  ...
  return (
    <text x={numX + numW / 2} y={numY - 4} textAnchor="middle" fill={fill} fontSize={9} fontWeight={600}>
      {fmtM(numValue, 1)}
    </text>
  );
}
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-3 text-[10px] text-[#6b7280]">
              <span className="flex items-center gap-1">
                <span className="w-2.5 h-2.5 rounded bg-[#1B5E20] inline-block" /> Presupuesto
              </span>
              <span className="flex items-center gap-1">
      ...
      <div className="mt-4 bg-white rounded-xl border border-[#e5e7eb] p-4">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[#e5e7eb]">
                    <th className="text-left py-2 px-2 text-[10px] font-semibold text-[#6b7280] uppercase">
                      Ejecutivo
                    </th>
        ...
        {cuentasSubTab === "pipeline" &&
          (() => {
            ...
            const totalCuentasValor = cuentas.reduce(
              (s, p) => s + (p.propuesta?.ventaTotal || p.facturacionEstimada || 0),
              0,
            );
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[10px] text-[#6b7280] ml-auto">
                    {cuentas.length} cuentas · {fmtM(totalCuentasValor)} total
                  </span>
                </div>
                {cuentas.length === 0 ? (
                            ...
                            <tr key={p.id} className="border-b border-[#f3f4f6] hover:bg-[#f9fafb] transition-colors">
                              ...
                              <td className="py-2 px-2">
                                <span
                                  ...
                                  style={{
                                    backgroundColor: `${stage?.color || "#6b7280"}15`,
...
function BudgetEditModal({
  ...
  const comercialMembers = salesTeamData.filter(
    (m) => m.presupuestoAnual2026 > 0 || Object.keys(m.presupuestosMensuales).length > 0,
  );
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e5e7eb]">
          <div>
            ...
            <p className="text-xs text-[#6b7280] mt-0.5">
              El total de cada mes se divide entre {memberCount} ejecutivos
            </p>
          </div>
              ...
              <div className="flex-1">
                {editingMonth === i ? (
                  ...
                  <div
                    onClick={() => {
                      setEditingMonth(i);
                      setEditText(monthValues[i] > 0 ? String(monthValues[i]) : "");
                    }}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-[#e5e7eb]">
          <span className="text-sm text-[#6b7280]">
            Total anual: <span className="font-bold text-[#1c2c4a]">{fmtM(grandTotal, 1)}</span>
          </span>
