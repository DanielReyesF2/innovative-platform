...
import { ExecutiveAvatar, KANBAN_STAGES, SERVICE_COLORS, SERVICIOS_INNOVATIVE } from "@/lib/comercial-constants";
...
export function PipelineTab({ onViewHub }: Props) {
  const { kanbanProspectos, salesTeamData } = useComercialData();
    ...
    <>
      ...
      {(() => {
        ...
        const totalFiltered = kanbanProspectos
          ...
          .filter((p) => filterEtapa === "todos" || p.status === filterEtapa).length;
              ...
              {salesTeamData.map((m) => {
                ...
                return (
                  <option key={m.codigo} value={m.codigo}>
                    {m.name.split(" ").slice(0, 2).join(" ")} ({count})
                  </option>
                );
              })}
            </select>
                    ...
                    {filteredProspectos.map((p) => {
                      ...
                      return (
                        <tr
                          ...
                          onClick={() => setSelectedProspectoId(p.id)}
                        >
                          <td className="px-4 py-2.5">
                            <div className="flex items-center gap-2">
                              <div
                                className="w-0.5 h-8 rounded-full flex-shrink-0"
                                style={{ backgroundColor: svc.border }}
                              />
                          ...
                          <td className="px-3 py-2.5">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-xs text-[#6b7280]">
                                {ejecutivo?.name?.split(" ").slice(0, 2).join(" ") || p.ejecutivo}
                              </span>
                            </div>
                          </td>
                          <td className="px-4 py-2.5">
                            ...
                            {p.contacto?.puesto && (
                              <div className="text-[11px] text-[#9ca3af] truncate">{p.contacto.puesto}</div>
                            )}
