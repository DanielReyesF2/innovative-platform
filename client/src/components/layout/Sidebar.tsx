...
import { LayoutDashboard, LogOut, type LucideIcon, Menu, X } from "lucide-react";
...
export function Sidebar() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: location triggers sidebar close on navigate
  useEffect(() => {
    setIsOpen(false);
  }, [location]);
        ...
        <nav className="flex-1 overflow-auto p-3">
          <ul className="space-y-1">
            {modules.map((mod) => {
              ...
              const isActive =
                mod.basePath === "/comercial"
                  ? location === "/" || location.startsWith("/comercial")
                  : mod.basePath === "/"
                    ? location === "/"
                    : location.startsWith(mod.basePath);
              return (
                <NavItem key={mod.name} href={mod.basePath} icon={Icon} label={mod.displayName} active={isActive} />
              );
            })}
          </ul>
        </nav>
          ...
          {user && (
            <div className="flex items-center gap-2.5 px-2 py-1.5 mb-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-sidebar-foreground truncate">{user.name || "Usuario"}</div>
