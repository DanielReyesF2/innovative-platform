...
async function runMigration() {
  ...
  console.log("Running CRM tables migration...");

  const statements = [
    // Create enums
    `DO $$ BEGIN CREATE TYPE "activity_type" AS ENUM('llamada', 'email', 'reunion', 'nota', 'cambio_etapa', 'documento', 'propuesta', 'otro'); EXCEPTION WHEN duplicate_object THEN null; END $$`,
    `DO $$ BEGIN CREATE TYPE "meeting_status" AS ENUM('programada', 'completada', 'cancelada', 'reprogramada'); EXCEPTION WHEN duplicate_object THEN null; END $$`,
    `DO $$ BEGIN CREATE TYPE "proposal_status" AS ENUM('borrador', 'enviada', 'revisada', 'aceptada', 'rechazada'); EXCEPTION WHEN duplicate_object THEN null; END $$`,
    `DO $$ BEGIN CREATE TYPE "alert_status" AS ENUM('pending', 'acknowledged', 'dismissed', 'auto_resolved'); EXCEPTION WHEN duplicate_object THEN null; END $$`,
    `DO $$ BEGIN CREATE TYPE "alert_type" AS ENUM('overdue_follow_up', 'stale_prospect', 'high_value_at_risk', 'scheduled_reminder'); EXCEPTION WHEN duplicate_object THEN null; END $$`,
